...
class Show {
    ...
    public static void log(long tempo, int comparacoes) {
        try (BufferedWriter esc = new BufferedWriter(new FileWriter("800712_hashIndireta.txt"))) {
...
class HashTable {
    private final int TAM_TAB = 21;
    private ArrayList<Show>[] tabela; 
    private int comparacoes;

    @SuppressWarnings("unchecked")
    public HashTable() {
        this.tabela = (ArrayList<Show>[]) new ArrayList[TAM_TAB];
    ...
    public boolean inserir(Show show) {
        ...
        int index = hash(show.getTitulo());
        tabela[index].add(show);
        return true;
    }
    ...
    public void pesquisar(String titulo) {
        ...
        for (Show s : tabela[index]) {
            comparacoes++;
            if (s.getTitulo().equals(titulo)) {
                encontrado = true;
                break; // Show encontrado, sai do loop
            }
        }
        ...
        if (encontrado)
            ...
            System.out.println(" (Posicao: " + index + ") NAO");
    }
}
