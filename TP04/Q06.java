...
class Show {
    ...
    public static void log(long tempo, int comparacoes) {
        try (BufferedWriter esc = new BufferedWriter(new FileWriter("800712_hashRehash.txt"))) {
...
class HashTable {
    private final int TAM_TAB = 21; // Tamanho da tabela hash
    private Show[] tabela;
    private int comparacoes;

    public HashTable() {
        this.tabela = new Show[TAM_TAB];
        for (int i = 0; i < TAM_TAB; i++) {
            this.tabela[i] = null; // Inicializa todas as posições como nulas
        }
        this.comparacoes = 0;
    }
    ...
    private int hash1(String titulo) {
        return getASCIISum(titulo) % TAM_TAB;
    }

    private int hash2(String titulo) {
        return (getASCIISum(titulo) + 1) % TAM_TAB;
    }
    ...
    public boolean inserir(Show show) {
        ...
        int idx1 = hash1(show.getTitulo());

        // Tenta inserir na primeira posição hash
        if (tabela[idx1] == null) {
            tabela[idx1] = show;
            return true;
        } else {
            // Colisão: tenta inserir na segunda posição hash (rehash)
            int idx2 = hash2(show.getTitulo());

            if (idx1 == idx2 || tabela[idx2] == null) {
                if (tabela[idx2] == null) {
                    tabela[idx2] = show;
                    return true;
                }
    ...
    public void pesquisar(String titulo) {
        int idx1 = hash1(titulo);
        boolean encontrado = false;
        int finalPosition = idx1; // Posição para imprimir se não for encontrado

        // 1. Procura na posição da primeira hash
        comparacoes++; 
        if (tabela[idx1] != null && tabela[idx1].getTitulo().equals(titulo)) {
            encontrado = true;
            finalPosition = idx1;
        }

        // 2. Se não encontrou na primeira posição, procura na posição de rehash
        if (!encontrado) { 
            int idx2 = hash2(titulo);
            comparacoes++;
            if (tabela[idx2] != null && tabela[idx2].getTitulo().equals(titulo)) {
                encontrado = true;
                finalPosition = idx2;
            }
        }
        ...
        if (encontrado) {
            System.out.println(" (Posicao: " + finalPosition + ") SIM");
        } else { 
            // Se não encontrou em nenhuma das duas posições
