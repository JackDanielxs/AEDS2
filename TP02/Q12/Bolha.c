...
void bubbleSortFilmes(int n) {
    Filme temp;
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - 1 - i; j++) {
            int precisaTrocar = 0;

            if (comparar(filmesId[j], filmesId[j + 1]) > 0) {
                comparacoes ++;
                precisaTrocar = 1;
            } else if (comparar(filmesId[j], filmesId[j + 1]) == 0 &&
...
int main() {
    ...
    FILE* arquivo = fopen("800712\_bolha.txt", "w");
