...
// Função de ordenação parcial (Insertion Sort nos 10 primeiros)
void insercaoParcial(int n) {
    for (int i = 1; i < n; i++) {
        Filme tmp = filmesId[i];
        movimentacoes++;
        int j = (i < 10) ? i - 1 : 10 - 1;
        ...
        while (j >= 0) {
            ...
            int cmpTipo = strcasecmp(filmesId[j].tipo, tmp.tipo);
            int cmpTitulo = strcasecmp(filmesId[j].titulo, tmp.titulo);

            // Se tipo for maior, ou se tipo igual e título maior → move para frente
            if (cmpTipo > 0 || (cmpTipo == 0 && cmpTitulo > 0)) {
                filmesId[j + 1] = filmesId[j];
                movimentacoes++;
                j--;
            } else {
                break; // posição correta
            }
        }
...
int main() {
    ...
    FILE* arquivo = fopen("800712_insercao.txt", "w");
