...
void insercaoPorCor(int n, int cor, int h) {
    for (int i = (h + cor); i < n; i += h) {
        ...
        int j = i - h;
        while (j >= 0 && (
            strcmp(filmesId[j].tipo, tmp.tipo) > 0 ||
            (strcmp(filmesId[j].tipo, tmp.tipo) == 0 && strcmp(filmesId[j].titulo, tmp.titulo) > 0)
        )) {
            filmesId[j + h] = filmesId[j];
            j -= h;
        }
        filmesId[j + h] = tmp;
    }
...
int main() {
    ...
    FILE* arquivo = fopen("800712\_shellsort.txt", "w");
