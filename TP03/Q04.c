...
void inserirFila(FilaCircular *fila, Show show) {
    if (((fila->fim + 1) % (TAM_FILA + 1)) == fila->inicio) {
        removerFila(fila);
    }
    fila->elementos[fila->fim] = show;
    fila->fim = (fila->fim + 1) % (TAM_FILA + 1);
...
Show removerFila(FilaCircular *fila) {
    ...
    fila->inicio = (fila->inicio + 1) % (TAM_FILA + 1);
    fila->soma_anos -= removido.release_year;
    fila->tamanho--;
    return removido;
}

void mostrarFila(FilaCircular *fila) {
    int i = fila->inicio, idx = 0;
    while (i != fila->fim) {
        imprimirShow(&fila->elementos[i], idx);
        i = (i + 1) % (TAM_FILA + 1);
        idx++;
    }
}
