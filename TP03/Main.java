...
class Lista {
    ...
    public static Lista filmesIds = new Lista(1368);
    ...
    public void manipularLinha(String linha) throws Exception {
        ...
        if (tipo.equals("II")) {
            ...
            Show s = Show.getById(id, Show.todosFilmes);
            if (s != null)
                filmesIds.inserir(s, pos);
        } else if (tipo.equals("IF")) {
            id = linha.substring(3);
            Show s = Show.getById(id, Show.todosFilmes);
            if (s != null)
                filmesIds.inserirFim(s);
        } else if (tipo.equals("RI")) {
            Show s = filmesIds.removerInicio();
            System.out.println("(R) " + s.getTitulo());
        } else if (tipo.equals("R*")) {
            String[] lista = linha.split(" ");
            pos = Integer.parseInt(lista[1].trim());
            Show s = filmesIds.remover(pos);
            System.out.println("(R) " + s.getTitulo());
        } else if (tipo.equals("RF")) {
            Show s = filmesIds.removerFim();
            System.out.println("(R) " + s.getTitulo());
        }
    }
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        while (!linha.equals("FIM")) {
            ...
            if (show != null)
                Lista.filmesIds.inserirFim(show);
        ...
        Lista.filmesIds.manipular(n, sc);

        for(int i = 0; i < Lista.filmesIds.n; i++){
            Lista.filmesIds.array[i].print();
        }
